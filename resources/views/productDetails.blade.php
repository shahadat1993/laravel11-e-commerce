...
<main class="pt-90">
      ...
      <div class="row">
        ...
        <div class="col-lg-5">
          ...
          <div class="product-single__addtolinks">
            ...
            <form action="{{ route('wishlist.item.remove',Cart::instance('wishlist')->content()->where('id', $product->id)->first()->rowId) }}" method="POST" id="wishlist_item_remove">
                @csrf
                @method('DELETE')
            <a href="javascript:void(0)" onclick="document.getElementById('wishlist_item_remove').submit()" class="menu-link menu-link_us-s add-to-wishlist filled-heart"><svg width="16" height="16" viewBox="0 0 20 20"
              ...
              <a href="javascript:void(0)" class="menu-link menu-link_us-s add-to-wishlist" onclick="document.getElementById('wishList_form').submit();"><svg width="16" height="16" viewBox="0 0 20 20"
                fill="none" xmlns="http://www.w3.org/2000/svg">
                <use href="#icon_heart" />
        ...
        <div class="swiper-container js-swiper-slider" data-settings='{
          ...
          <div class="swiper-wrapper">
            ...
            <div class="swiper-slide product-card">
              ...
              <a href="{{ route('cart') }}" class="pc__atc btn anim_appear-bottom btn position-absolute border-0 text-uppercase fw-medium btn-warning mb-3">Go to cart</a>
          @else
           <form name="addtocart-form" method="post" action="{{ route('cart.add') }}">
            @csrf
            <input type="hidden" name="product_id" value="{{ $rproduct->id }}">
              ...
              <div class="pc__info position-relative">
                ...
                <h6 class="pc__title"><a href="{{ route('shop.details',$rproduct->slug) }}">{{ $rproduct->name }}</a></h6>
                <div class="product-card__price d-flex">
                  <span class="money price">
                    @if ($rproduct->sale_price)
                        <s style="margin-right: 6px">{{ $rproduct->regular_price }}  </s>{{ $rproduct->sale_price }}
                    @else
    ...
    <form method="POST" action="{{ route('wishlist.item.remove', Cart::instance('wishlist')->content()->where('id', $rproduct->id)->first()->rowId) }}">
        @csrf
        @method('DELETE')
        <button
            type="submit"
